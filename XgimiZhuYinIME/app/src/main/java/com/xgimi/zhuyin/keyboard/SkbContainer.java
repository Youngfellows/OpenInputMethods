...
public class SkbContainer extends RelativeLayout implements SoftKeyBoardable {
	...
	public void setSoftKeySelectPadding(RectF rect) {
		mSoftKeyboardView.setSoftKeySelectPadding(rect);
	}
	...
	public int getSelectRow() {
		if (mSoftKeyboardView != null) {
			SoftKeyboard softKeyboard = mSoftKeyboardView.getSoftKeyboard();
			return softKeyboard.getSelectRow();
		}
		return -1;
	}
	...
	public int getSelectIndex() {
		if (mSoftKeyboardView != null) {
			SoftKeyboard softKeyboard = mSoftKeyboardView.getSoftKeyboard();
			return softKeyboard.getSelectIndex();
		}
		return -1;
	}
...
//		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (downSKey != null) {
				...
				//
				if (mSoftKeyListener != null) {
					mSoftKeyListener.onTouchEventDown(event, downSKey);
				}
			}
			break;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		SoftKeyboard softKeyboard = mSoftKeyboardView.getSoftKeyboard();
		if (softKeyboard != null) {
			measuredHeight += softKeyboard.getHeight();
		}
	...
	public SoftKey getSoftKey(int rIndex, int cIndex) {
		SoftKeyboard softKeyboard = mSoftKeyboardView.getSoftKeyboard();
		if (softKeyboard != null) {
			return softKeyboard.getSoftKey(rIndex, cIndex);
		}
		return null;
	}
