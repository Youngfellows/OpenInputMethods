...
import com.xgimi.zhuyin.keyboard.SoftKey.SaveSoftKey;
...
public class SoftKeyboardView extends View {
	...
	private void drawSoftKeySelectState(Canvas canvas, SoftKey softKey) {
		...
		if (selectDrawable != null) {
			...
			int left = (int) Math.rint(mSelectRect.left);
			int top = (int) Math.rint(mSelectRect.top);
			int right = (int) Math.rint(mSelectRect.right);
			int bottom = (int) Math.rint(mSelectRect.bottom);
			rect = new Rect(rect.left - left, rect.top - top, rect.right + right, rect.bottom + bottom);
	...
	private RectF mSelectRect = new RectF(0, 0, 0, 0);
	
	/**
	 * 设置移动边框的相差的间距.
	 */
	public void setSoftKeySelectPadding(float padding) {
				...
				if (softKey == null) {
					...
					if (currentRow > (mSoftKeyboard.getRowNum() - 1)) {
						if (!mSoftKeyboard.isTBMove()) {
							return true; // 因为输入法这里需要修改为 true,不能为false，不然事件会被传递下去.
						}
